...
test('Should be able to return the latest time when specifying a latest time within a file', async function(t) {
  const dbInstance = Object.create(FsTimeSeriesDB).setOptions({rootPath: rootPath});
  ...
  const putResult1 = await dbInstance.putEvent({
    ...
    endTimeMilliSec: events1[2].epochTimeMilliSec,
  }, events1);
  t.ok(putResult1, 'put result is truthy for events1');
  ...
  const putResult2 = await dbInstance.putEvent({
    ...
    group2, group2,
    epochTimeMilliSec: events1[0].epochTimeMilliSec,
    endTimeMilliSec: events2[2].epochTimeMilliSec,
  }, events2);
  t.ok(putResult2, 'put result is truthy for events2');
  ...
  const putResult3 = await dbInstance.putEvent({
    ...
    endTimeMilliSec: events3[2].epochTimeMilliSec,
  }, events3);
  t.ok(putResult3, 'put result is truthy for events3');
